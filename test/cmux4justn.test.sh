...
WT_STDOUT="$TMPDIR/wt.stdout"
WT_STDERR="$TMPDIR/wt.stderr"
...
"$CLI" worktree --apply >"$WT_STDOUT" 2>"$WT_STDERR"
[ "$(cat "$WT_STDOUT")" = "$WORKTREE_ROOT_RESOLVED/cmux4justn-main" ] || fail "worktree apply should print only final path"
assert_contains "$(cat "$WT_STDERR")" "create-worktree	cmux4justn-main	$WORKTREE_ROOT_RESOLVED/cmux4justn-main	worktree/cmux4justn-main"
...
"$CLI" worktree --apply >"$WT_STDOUT" 2>"$WT_STDERR"
[ "$(cat "$WT_STDOUT")" = "$WORKTREE_ROOT_RESOLVED/cmux4justn-main-2" ] || fail "second worktree apply should print only final path"
assert_contains "$(cat "$WT_STDERR")" "create-worktree	cmux4justn-main-2	$WORKTREE_ROOT_RESOLVED/cmux4justn-main-2	worktree/cmux4justn-main-2"
[ -d "$WORKTREE_ROOT_RESOLVED/cmux4justn-main-2" ] || fail "second worktree apply should create a suffixed worktree"

"$CLI" worktree --apply --name api >"$WT_STDOUT" 2>"$WT_STDERR"
[ "$(cat "$WT_STDOUT")" = "$WORKTREE_ROOT_RESOLVED/api" ] || fail "named worktree apply should print only final path"
assert_contains "$(cat "$WT_STDERR")" "create-worktree	api	$WORKTREE_ROOT_RESOLVED/api	worktree/api"
[ -d "$WORKTREE_ROOT_RESOLVED/api" ] || fail "named worktree apply should create explicit worktree"

"$CLI" worktree --apply --name prune-me >"$WT_STDOUT" 2>"$WT_STDERR"
[ "$(cat "$WT_STDOUT")" = "$WORKTREE_ROOT_RESOLVED/prune-me" ] || fail "prune worktree apply should print only final path"
assert_contains "$(cat "$WT_STDERR")" "create-worktree	prune-me	$WORKTREE_ROOT_RESOLVED/prune-me	worktree/prune-me"
...
"$CLI" wt for-feature1 >"$WT_STDOUT" 2>"$WT_STDERR"
[ "$(cat "$WT_STDOUT")" = "$WORKTREE_ROOT_RESOLVED/for-feature1" ] || fail "positional wt should print only final path"
assert_contains "$(cat "$WT_STDERR")" "create-worktree	for-feature1	$WORKTREE_ROOT_RESOLVED/for-feature1	worktree/for-feature1"
...
"$CLI" wt --repo "$OTHER_REPO" other-feature >"$WT_STDOUT" 2>"$WT_STDERR"
[ "$(cat "$WT_STDOUT")" = "$OTHER_WORKTREE_ROOT_RESOLVED/other-feature" ] || fail "other repo worktree should print only final path"
assert_contains "$(cat "$WT_STDERR")" "create-worktree	other-feature	$OTHER_WORKTREE_ROOT_RESOLVED/other-feature	worktree/other-feature"
...
assert_contains "$output" "c4j wt path-only stdout contract"
...
NEXT_MINOR_VERSION="$CURRENT_MAJOR.$((CURRENT_MINOR + 1)).0"
output="$(bash "$ROOT/scripts/release.sh" --dry-run "$NEXT_MINOR_VERSION")"
assert_contains "$output" "release-dry-run	$CURRENT_VERSION	$NEXT_MINOR_VERSION"
assert_contains "$output" "would-create-release	v$NEXT_MINOR_VERSION"
