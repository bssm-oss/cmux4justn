...
```bash
curl -fsSL https://raw.githubusercontent.com/bssm-oss/cmux4justn/v0.14.1/install.sh | bash -s -- --rc
```
...
기본 실행은 worktree를 만든 뒤 그 worktree용 cmux workspace도 생성하거나 선택합니다. 기본 workspace 이름은 `@active/<repo>-<worktree>`이고, 이미 해당 worktree cwd를 가리키는 workspace가 있으면 그것도 재사용합니다. `--workspace-name`으로 이름을 직접 지정하고, `--name-prefix`로 workspace prefix를 바꾸고, `--cmux`로 cmux binary를 지정할 수 있으며, `--no-cmux`로 cmux 연동을 끌 수 있습니다.

일반 실행의 stdout은 최종 worktree 절대경로 한 줄뿐입니다. 상태 row, cmux warning, `--command` 출력은 stderr로 갑니다. 이 계약 덕분에 `scripts/install.sh --rc`로 설치한 shell wrapper가 경로를 읽고 현재 shell을 생성/재사용한 worktree로 옮길 수 있습니다.

`--dry-run`은 별도 모드입니다. filesystem이나 cmux를 바꾸지 않고, stdout에는 TSV action row를 출력합니다. worktree 생성 dry-run row는 target type, target, would-change, reason 필드를 포함하고, move/delete/update 같은 subcommand는 각 action에 맞는 TSV row shape을 유지합니다.

`--command`는 setup 뒤 worktree 안에서 실행됩니다. command가 실패하면 `c4j wt`도 그 exit code를 반환하지만 stdout은 최종 worktree 경로 한 줄로 유지합니다.

`c4j wt list`(`ls`)는 cmux 워크스페이스 안에서는 현재 워크스페이스의 worktree만 보여주고, 워크스페이스 밖에서는 찾을 수 있는 전체 managed worktree를 보여줍니다. `--repo`로 repo를 좁히고, `--plain`/`--tsv`로 raw row를 출력할 수 있습니다.
`c4j wt prune`은 현재 스코프나 `--repo` 스코프의 stale worktree 메타데이터를 정리합니다.
`c4j wt move`는 지정한 worktree를 새 이름/경로로 옮깁니다. managed non-main worktree 안에서 실행하면 `c4j wt move <destination>`처럼 현재 worktree 이름을 생략할 수 있습니다. `--target`, `--to`, `--destination`이 명시형 옵션입니다.
`c4j wt delete`(`remove`, `rm`)는 지정한 worktree를 제거합니다. managed non-main worktree 안에서는 이름을 생략할 수 있습니다. dirty 또는 untracked worktree는 여전히 `--force` 또는 `--discard`가 필요합니다.
`c4j wt update`(`refresh`, `up`)는 지정한 worktree나 현재 worktree를 fetch 후 fast-forward합니다. 명시적으로 고를 때는 `--target`을 씁니다.
...
c4j wt move api-v2        # 현재 worktree 안에서 실행
c4j wt delete --discard   # 현재 worktree 안에서 실행
...
- 새 작업 브랜치용 worktree를 만들고 바로 들어가기:
  ```bash
  c4j wt feature-api
  ```
  성공하면 stdout은 worktree 절대경로 한 줄뿐이라 shell wrapper가 그 위치로 `cd`합니다.

- 실제 변경 전에 어떤 일이 일어날지 확인하기:
  ```bash
...
- 현재 worktree 안에서 이름 없이 정리하기:
  ```bash
  c4j wt move feature-api-v2
  c4j wt delete --discard
  c4j wt update
  ```
  이 단축형은 managed non-main worktree 안에서만 동작합니다. main checkout에서는 삭제를 거부하고, dirty/untracked worktree 삭제는 `--force` 또는 `--discard` 없이는 거부합니다.

- script나 자동화에서 대상 repo를 고정하기:
  ```bash
  c4j wt list --repo ~/Workspaces/repos/example --plain
...
```bash
# 특정 릴리즈를 설치합니다.
curl -fsSL https://raw.githubusercontent.com/bssm-oss/cmux4justn/v0.14.1/install.sh | C4J_REF=v0.14.1 bash -s -- --rc
...
curl -fsSL https://raw.githubusercontent.com/bssm-oss/cmux4justn/v0.14.1/install.sh | C4J_INSTALL_DIR="$HOME/src/c4j" bash -s -- --rc
