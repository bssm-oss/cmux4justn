...
```bash
curl -fsSL https://raw.githubusercontent.com/bssm-oss/cmux4justn/v0.13.19/install.sh | bash -s -- --rc
```
...
기본 실행은 worktree를 만든 뒤 그 worktree용 cmux workspace도 생성하거나 선택합니다. 기본 workspace 이름은 `@active/<repo>-<worktree>`이고, 이미 해당 worktree cwd를 가리키는 workspace가 있으면 그것도 재사용합니다. `--workspace-name`으로 이름을 직접 지정할 수 있고, `--no-cmux`로 cmux 연동을 끌 수 있습니다.
...
`--dry-run`은 별도 모드입니다. filesystem이나 cmux를 바꾸지 않고, stdout에는 TSV action row를 출력합니다. worktree dry-run row는 action, target type, target, 변경 여부, reason을 포함합니다.
...
```bash
# 특정 릴리즈를 설치합니다.
curl -fsSL https://raw.githubusercontent.com/bssm-oss/cmux4justn/v0.13.19/install.sh | C4J_REF=v0.13.19 bash -s -- --rc
...
curl -fsSL https://raw.githubusercontent.com/bssm-oss/cmux4justn/v0.13.19/install.sh | C4J_INSTALL_DIR="$HOME/src/c4j" bash -s -- --rc
